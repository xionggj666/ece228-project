...
def main():
    for sub in subjectList:
        FFT_Processing (sub, channel, band, window_size, step_size, sample_rate, input_path, output_path)
    X_train, y_train, X_test, y_test = load_data(subjectList, output_path,label_index=3, samples_per_trial=464)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = CNN_1D(num_classes=num_classes).to(device)
    pipeline = Pipeline(model, X_train, y_train, X_test, y_test,mode = '1D')
    pipeline.train()
